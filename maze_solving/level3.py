    ...
    if ( ang == 0 ) :
        if ( ( x  , y + blockSize ) in wall ) :
            if ( ( x + blockSize , y ) not in wall ) :
                ...
                x = currentx()
                y = currenty()
                if ( ( x , y ) in coins ) and ( ( x , y ) not in array_coordinates )  :
                    counter +=1
                    array_coordinates.append( ( x , y ) )
                time.sleep(0.001)
            else :
                right()
                time.sleep(0.001)
...
while(True):
    ...
    * To end the game break from the while loop
    ...
    if( ( x , y ) == finish):
        finish
        print ( " number of coins : " + str(counter) )
        for a in range(len(array_coordinates)):
            print ( " coordinate of coin #" + str(a+1) + ":" + str(array_coordinates[a]) )
        break
