...
while(True):
    ...
    * To end the game break from the while loop
    ...
    if ( ( x  , y - blockSize ) not in wall ) and ( ( x  , y - blockSize ) not in list ) :
        if ( ang == 0 ) :
            right()
        if ( ang == 90 ) :
            right()
    ...
    y = currenty()
    ang = character.getAngle()
    if ( ( x + blockSize , y  ) not in wall ) and ( ( x + blockSize , y ) not in list ) :
        if ( ang == 0 ) :
            forward()
            x = currentx()
            y = currenty()
            path.drawBlock(x - blockSize,y)
            list.append( ( x - blockSize , y ) )
        if ( ang == 90 ) :
            right()
        if ( ang == 180 ) :
            left()
